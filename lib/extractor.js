...
export async function extractTranscript(videoId, lang = "en") {
  // Strategy 1: Scrape the watch page HTML
  let data = await tryWatchPage(videoId);

  // Strategy 2: WEB InnerTube client
  if (!data) {
    data = await tryInnerTube(videoId, WEB_CONTEXT, BROWSER_UA);
  }

  // Strategy 3: ANDROID InnerTube client
  if (!data) {
    data = await tryInnerTube(videoId, ANDROID_CONTEXT, ANDROID_UA);
  }
